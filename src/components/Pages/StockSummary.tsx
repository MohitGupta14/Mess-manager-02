// ===== FILE: src/components/Pages/StockSummary.tsx =====
'use client';
...
export default function StockSummary({ displayModal }: StockSummaryProps) {
  const { data: stockItems, deleteData } = useMessData('stockItems');
  const { data: minStockLevels } = useMessData('minStockLevels');
  const [editingItem, setEditingItem] = useState<StockItem | null>(null);

  const nonLiquorItems = stockItems.filter(item => item.itemType !== 'Liquor');
  const sortedItems = [...nonLiquorItems].sort((a, b) => a.itemName.localeCompare(b.itemName));
