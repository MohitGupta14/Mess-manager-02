// ===== FILE: src/components/Pages/BarCounter.tsx =====
'use client';
...
export default function BarCounter({ displayModal }: BarCounterProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (stockItem.currentQuantity < qty) {
      displayModal(`Not enough ${wineType} in stock!`, 'error');
      return;
    }
  ...
  return (
    <div className="p-6 sm:p-8 rounded-xl w-full bg-white shadow-lg">
      <h1 className="text-2xl sm:text-3xl font-bold text-center text-gray-800 mb-8">Bar Counter</h1>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <form onSubmit={handleSubmit} className="space-y-6 bg-gray-50 p-6 rounded-lg shadow-md">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Liquor / Wine <span className="text-red-500">*</span></label>
            <select 
              value={wineType} 
              ...
              {messMembers.map(member => (
                <div 
                  ...
                  className={`p-3 text-center rounded-lg cursor-pointer border-2 transition-all duration-200 ${
                    sharingMembers.includes(member.memberId) 
        ...
        <div className="bg-gray-50 p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold text-gray-800 mb-4 text-center">Recent Entries</h2>
