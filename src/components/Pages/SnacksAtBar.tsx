...
export default function SnacksAtBar({ displayModal }: SnacksAtBarProps) {
  ...
  const { data: snacksAtBarEntries, addData: addSnack } =
    useMessData("snacksAtBarEntries");
  ...
  const areAllFilteredSelected =
    ...
    filteredMembers.every((m: MessMember) => sharing.includes(m.memberId));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (currentStock < quantity) {
      displayModal(
        `Not enough ${item} in stock! Available: ${currentStock}`,
        "error"
      );
      return;
    }
        ...
        >
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Snack Item <span className="text-red-500">*</span>
            </label>
            <select
              value={item}
          ...
          <div>
            ...
            <div>
              <span className="text-gray-500 text-xs">
                Available:{" "}
                {snackItems.find((i) => i.itemName === item)?.currentQuantity ||
                  0} {" "}
                 {snackItems.find((i) => i.itemName === item)?.unitOfMeasurement ||
                  ""}
              </span>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2 gap-2">
              <label className="block text-sm font-bold text-gray-800">
                Sharing Members{" "}
                <span className="text-blue-600">({sharing.length})</span>{" "}
        ...
        <div className="bg-gray-50 p-6 rounded-lg shadow-md">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
            <h2 className="text-xl font-semibold text-gray-800 mb-4">
              ...
              <PrintExportButtons
                tableId="snacksAtBarTable"
                filename="snacks-at-bar-history"
                title="Snacks at Bar History"
              />
          </div>
          <div
            id="snacksAtBarTable"
            className="overflow-y-auto max-h-[70vh] border rounded-lg bg-white shadow-sm"
          >
